...
工作区隶属于工作台（个人或组织）。工作区是建模单位，只认内容、不认位置——工件在物理上从哪来、落在哪，由平台在装载时决定；同一个工作区，资源可以来自多处、任意组合。
...
- `id`(UUID): 唯一标识
- `name`(Slug): 工作区名称，用于编排、URL 等；在其所属工作台内唯一
- `title`(String): 展示名称，可选，缺省时取 `name`
- `workbench_id`(UUID): 所属工作台（个人或组织）
- `is_active`(Bool): 是否处于可用状态；目前固定为 `true`
- `created_at`(Datetime): 创建时间
- `updated_at`(Datetime): 最近更新时间

工作区与工件、过程的关联：

- 一个工作区包含零至多个工作流（Workflow）、任务（Task）、产物（Artifact）、材料（Material）；
- 上述对象必须归属于且仅归属于一个工作区，工作区是它们的作用域；
- 关联通过子资源端点暴露与遍历，不作为内嵌字段随工作区响应返回。

### 不变式

1. **归属唯一**：任何材料、产物、工作流、任务必须且只能归属于一个工作区；
2. **区内互见**：同一工作区内的工件与过程彼此可见、可相互引用与流转；
3. **区间隔绝**：工作区之间默认互不可见；跨区引用须经显式的导出／导入操作；
4. **可用性前提**：工作区处于可用状态（`is_active = true`）时，其内材料方可装载、任务方可流转、产物方可产出与验收。
...
- **前置性**：工作区已创建先于其内一切事件——材料已收录、工作流已创建、任务已启动／已完成、产物已生成／已验收，均以工作区已创建为前提；
