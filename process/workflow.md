...
工作流是一份 YAML，落在工作区的 `workflows/<工作流>.yaml`，文件名即工作流名。
...
- **判据里的路径在不在**：取 `path` 与 `file` 两类 `rule` 判据里写的路径，把占位换成工作区内的实际位置，再看它在不在。带 `{{report}}`、`{{journal}}`、`{{log}}`、`{{artifacts}}` 占位的判据跳过——那几处要等任务执行时才落，核对时给一条「未核」的回执。
