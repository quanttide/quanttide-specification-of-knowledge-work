...
定义除了语法，还要能对着数据仓核一遍（`workflow --check` 一类命令）。核两件事：

- **判据里的路径在不在**：取 `path` 与 `file` 两类 `rule` 判据里写的路径，把占位换成数据仓内的实际位置，再看它在不在。带 `{{report}}`、`{{journal}}`、`{{log}}`、`{{artifacts}}` 占位的判据跳过——那几处要等任务执行时才落，核对时给一条「未核」的回执。
