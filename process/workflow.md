...
判据里的路径指向区内工件，相对路径的基准由平台给出。路径里可写四个占位，工单执行时换成本次行程的落点：`{{report}}`、`{{journal}}` 是这两样产物的落点，`{{log}}` 是工单的流水，`{{artifacts}}` 是产物目录。占位只认这四个，写了别的视为不合语法。
...
- 带占位的路径要等工单执行时才落，核对记「未核」；核对不访问文件系统，在不在由端侧判断。
